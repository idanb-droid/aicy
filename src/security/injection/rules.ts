...
export const INJECTION_RULES: InjectionRule[] = [
  ...
  { id: "A-unrestricted", category: "A", description: "Unrestricted/debug/jailbreak mode", pattern: /\b(unrestricted|debug|jailbreak|god)\s+mode\b/ },
  ...
  { id: "B-secret-exfil", category: "B", description: "Probe for env vars / API keys", pattern: /(environment variables?|configuration values?|api keys?).{0,60}(output|print|dump|reveal|as json|now|beginning with|you can see)/ },

  // C — role / persona hijack
  { id: "C-dan", category: "C", description: "DAN-style persona jailbreak", pattern: /do anything now|\[dan\]:|dan\s*\(/ },
  { id: "C-no-restrictions", category: "C", description: "Persona with no restrictions", pattern: /\b(dan|persona|character|mode|it)\s+has\s+(no|without)\s+(restrictions|filtering|limits|safety)|(no|without)\s+(restrictions|filtering|limits|safety)\s+and\s+(answer|respond|bypass|follow)/ },
