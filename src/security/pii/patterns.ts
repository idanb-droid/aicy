...
export const PII_PATTERNS: PiiPattern[] = [
  ...
  // NOTE: no /g flag — callers must build a global copy for replacement (avoids lastIndex corruption).
  ...
  { category: "PHONE", regex: /(?:\+\d{1,3}[-\s])?(?:\(?\d{1,4}\)?[-\s]){2,4}\d{2,4}/ },
  // Israeli national ID: a standalone run of exactly 9 digits.
  { category: "ISR_ID", regex: /(?<!\d)\d{9}(?!\d)/ },
];
